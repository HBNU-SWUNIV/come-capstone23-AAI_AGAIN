...
class SavePeftModelCallback(transformers.TrainerCallback):
    def __init__(self):
        super().__init__()
        self.data_pointer = 17700

    def on_step_end(self, args, state, control, **kwargs):
        # 매 스텝이 끝날 때마다 데이터 포인터를 갱신합니다. 
        self.data_pointer += args.per_device_train_batch_size
    ...
    def save_model(self, args, state, kwargs):
        ...
        # 데이터 포인터 위치를 저장합니다.
        with open(os.path.join(peft_model_path, "data_pointer.txt"), "w") as f:
            f.write(str(self.data_pointer))
...
def main():
    ...
    if os.path.isdir(training_args.output_dir) and training_args.do_train and not training_args.overwrite_output_dir:
        ...
        if last_checkpoint is None and len(os.listdir(training_args.output_dir)) > 0:
            raise ValueError(
                ...
                f"데이터 포인트 {training_args.data_point}부터 학습을 재개합니다."
    ...
    with training_args.main_process_first(desc="dataset map tokenization and grouping"):
        ...
        lm_datasets = lm_datasets.train_test_split(test_size = data_args.validation_split_percentage, shuffle=True)
    ...
    if training_args.peft_path is not None:
        print('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
        print('Peft from pre-trained model')
        print('@@@@@@@@@@@@@@@@@@@@@@2@@@@@')
    ...
    print('training_args.data_point', training_args.data_point)
